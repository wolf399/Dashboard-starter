...
export const build = async () => {
  ...
  await fastify.register(swagger, { openapi: { info: { title: 'Agent CRM API', version: '1.0.0', description: 'Agent CRM API' }, components: { securitySchemes: { bearerAuth: { type: 'http', scheme: 'bearer', bearerFormat: 'JWT' } } } } });
...
const setupGracefulShutdown = (fastify: Fastify.FastifyInstance) => {
  ['SIGTERM', 'SIGINT'].forEach((signal) => {
    process.on(signal, async () => {
      try { await fastify.close(); process.exit(0); } catch { process.exit(1); }
    });
  });
};
...
const start = async () => {
  ...
  try {
    await fastify.listen({ port: Number(process.env.PORT) || 3000, host: process.env.NODE_ENV === 'production' ? '0.0.0.0' : '127.0.0.1' });
