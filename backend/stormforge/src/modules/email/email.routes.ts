...
// ── Send a reply via the org's connected Gmail account ──
export const sendEmailViaGmail = async (
  ...
  const emailLines = [
    ...
    `<p style="color:#9ca3af;font-size:12px;">Ticket ID: ${ticket?.id} — Replied via Agent CRM</p>`,
    `</div>`,
  ];
...
export default async function emailRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/send', async (request: any, reply: any) => {
    ...
    try {
      ...
      const result = await sendEmailViaGmail(org, ticket, to, subject, text);
      return result;
    } catch (err: any) {
